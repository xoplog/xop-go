...
func New(app *nserve.App, creg *nfigure.Registry) *server {
	...
	app.On(nserve.Start, func(app *nserve.App) error {
		...
		for _, addr := range s.ListenAddress {
			...
			if err != nil { return errors.Wrapf(err, "xoplog log recevier at %s", addr) }
			go func() {
				err := http.Serve(listener, handler)
				if err != nil && !shutdownCalled {
					panic(fmt.Sprintf("http.Serve for xoplog log recevier on %s exited early with %s", addr, err))
				}
			}
		}
		okay = true
...
func (s *server) GetHandler() http.Handler {
	r := mux.NewRouter()
	service := npoint.RegisterServiceWithMux("xoplog-receive", r,
		LOGGER,
