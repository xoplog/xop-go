...
func (span *Span) SubSeed(mods ...SeedModifier) Seed {
	...
	if !span.seed.spanSet {
		n.traceBundle.Trace.SpanID().SetRandom()
		fmt.Println("XXX Seed.SubSeed(), span set to", n.traceBundle.Trace.SpanID())
	} else {
		fmt.Println("XXX Seed.SubSeed(), span remains", n.traceBundle.Trace.SpanID())
	}
	n = n.applyMods(mods)
	return n
}
...
// trace state and baggage.
func WithBundle(bundle xoptrace.Bundle) SeedModifier {
	return func(s *Seed) {
		s.traceBundle = bundle
		s.traceSet = true
		s.spanSet = true
		fmt.Println("XXX WithBundle, trace is", bundle.Trace, "parent is", bundle.Parent)
	}
}
...
func WithSpan(spanID [8]byte) SeedModifier {
	return func(s *Seed) {
		...
		fmt.Println("XXX WithSpan, set to", s.traceBundle.Trace.SpanID())
		s.spanSet = true
	}
}
...
func (seed Seed) react(isRequest bool, description string, now time.Time) Seed {
	...
	fmt.Println("XXX react done, trace now", seed.traceBundle.Trace)
	return seed
}
