...
func (logger *Logger) Request(ts time.Time, trace trace.Bundle, name string) xopbase.Request {
	...
	rq.dataBuffer.AppendBytes([]byte(`{"type":"request","trace.id":"`))
	...
	rq.Time("ts", ts)
	rq.dataBuffer.AppendBytes([]byte{'}', '\n'})
	if logger.perRequestBufferLimit != 0 {
	...
	go func() {
		for {
			...
			case builder := <-r.completedBuilders:
				if len(builder.dataBuffer.B)+len(r.writeBuffer) > r.logger.perRequestBufferLimit {
					r.flushBuffer()
				}
				if len(builder.dataBuffer.B) > r.logger.perRequestBufferLimit {
					// TODO: split into multiple writes
				}
				r.writeBuffer = append(r.writeBuffer, builder.dataBuffer.B...)
...
func (b *builder) reset(s *span) {
	b.span = s
	b.dataBuffer.B = b.dataBuffer.B[:0]
...
func (p *prefilled) Line(level xopconst.Level, t time.Time, pc []uintptr) xopbase.Line {
	...
	case SpanIDTagOption:
		...
		l.dataBuffer.SafeString(l.span.trace.Trace.SpanIDString())
	case FullIDTagOption:
		l.dataBuffer.Comma()
		l.dataBuffer.AppendBytes([]byte(`"trace.header":`))
		l.dataBuffer.SafeString(l.span.trace.Trace.HeaderString())
	case TraceIDTagOption:
		l.dataBuffer.Comma()
		l.dataBuffer.AppendBytes([]byte(`"trace.id":`))
		l.dataBuffer.SafeString(l.span.trace.Trace.TraceIDString())
