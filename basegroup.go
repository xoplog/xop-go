...
func (l baseLoggers) SetErrorReporter(f func(error)) {
	for _, logger := range l {
...
func (s baseSpans) Line(level xopconst.Level, t time.Time, forkPrefix string, pc []uintptr) xopbase.Line {
	lines := make(lines, len(s))
	for i, span := range s {
		lines[i] = span.Line(level, t, forkPrefix, pc)
	}
	return lines
}

func (l lines) Recycle(level xopconst.Level, t time.Time, forkPrefix string, pc []uintptr) {
	for _, line := range l {
		line.Recycle(level, t, forkPrefix, pc)
	}
}
