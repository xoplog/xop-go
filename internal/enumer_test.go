...
func TestEnumerDataType(t *testing.T) {
	values := xopbase.DataTypeValues()
	vlast := values[len(values)-1]
	assert.NotEmpty(t, vlast.String(), "valid")
	assert.NotEmpty(t, (vlast + 1).String(), "invalid")
	for _, s := range xopbase.DataTypeStrings() {
		v, err := xopbase.DataTypeString(s)
		assert.NoError(t, err, s)
		assert.Equal(t, s, v.String())
	}
	v, err := xopbase.DataTypeString(vlast.String())
	require.NoError(t, err, "identity")
	assert.Equal(t, vlast, v, "identity")
	v, err = xopbase.DataTypeString(strings.ToLower(vlast.String()))
	require.NoError(t, err, "identity, lower")
	assert.Equal(t, vlast, v, "identity, lower")
	_, err = xopbase.DataTypeString("lasjf;asjfl;adsjf;lasdjfl;jasdf")
	assert.Error(t, err, "invalid")
	assert.True(t, vlast.IsADataType(), "is valid")
	assert.False(t, (vlast + 1).IsADataType(), "is not valid")
	enc, err := json.Marshal(vlast)
	require.NoError(t, err, "marshal")
	require.NotEmpty(t, enc, "enc")
	var unenc xopbase.DataType
