...
func (log *TestLogger) SetPrefix(p string) {
	log.linePrefix = p
}

func (log *TestLogger) output(s string) {
	log.t.Log(log.linePrefix + s)
}
...
func (span *Span) setShortRequest() {
	...
	if ti, ok := span.testLogger.traceMap[ts]; ok {
		...
		span.testLogger.output("Start request " + span.Short + "=" + span.Bundle.Trace.String() + " " + span.Name)
		return
	}
...
func (span *Span) setShortSpan() {
	...
	span.testLogger.output("Start span " + span.Short + "=" + span.Bundle.Trace.String() + " " + span.Name)
}
...
func (line Line) send(text string) {
	line.Span.testLogger.output(text)
