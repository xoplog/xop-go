			...
			for _, linkLine := range span.Links {
				...
				if tssRaw, ok := linkLine.Data[xopOTELLinkTranceState]; ok && linkLine.DataType[xopOTELLinkTranceState] == xopbase.StringDataType {
					...
					if ok {
						...
						linkLine.Data[xopLinkTraceStateError] = "attribute " + xopOTELLinkIsRemote + " should be a string"
						linkLine.DataType[xopLinkTraceStateError] = xopbase.ErrorDataType
					}
				}
				if tssRaw, ok := linkLine.Data[xopOTELLinkIsRemote]; ok && linkLine.DataType[xopOTELLinkIsRemote] == xopbase.BoolDataType {
					var ok bool
					isRemote, ok = tssRaw.(bool)
					fmt.Println("XXX BUFFERED isRemote", isRemote, ok)
					if ok {
						delete(linkLine.Data, xopOTELLinkIsRemote)
						delete(linkLine.DataType, xopOTELLinkIsRemote)
					} else {
						linkLine.Data[xopLinkTraceStateError] = "attribute " + xopOTELLinkIsRemote + " should be a bool"
						linkLine.DataType[xopLinkTraceStateError] = xopbase.ErrorDataType
					}
				}
				fmt.Printf("XXX BUFFERED LINKLINE %+v\n", linkLine.Data)
