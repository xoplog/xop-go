	...
	for _, a := range attributes {
		...
		case xopLineFormat:
			if a.Value.Type() == attribute.STRING {
				...
				default:
					return nil, errors.Errorf("invalid line format attribute value %s", a.Value.AsString())
				}
			} else {
				return nil, errors.Errorf("invalid line format attribute type %s", a.Value.Type())
			}
		case xopTemplate:
			if a.Value.Type() == attribute.STRING {
